...
RFM (Recency, Frequency, Monetary) analysis is a powerful technique for segmenting suppliers based on their procurement behavior. This section implements a comprehensive RFM analysis of the GeBiz procurement dataset.

### 7.1 Computing RFM Metrics

First, we'll calculate the three core RFM metrics for each supplier in our dataset:

```{r}
# Set analysis date (latest date in dataset + 1 day for accurate recency calculation)
if(!"award_date" %in% colnames(procurement_enriched) || !inherits(procurement_enriched$award_date, "Date")) {
  procurement_enriched <- procurement_enriched %>%
    mutate(award_date = as.Date(award_date, format="%d/%m/%Y"))
}

analysis_date <- max(procurement_enriched$award_date, na.rm = TRUE) + days(1)

# Calculate RFM metrics for each supplier
supplier_rfm <- procurement_enriched %>%
  group_by(supplier_name) %>%
  summarize(
  ...
  filter(!is.na(supplier_name), supplier_name != "Unknown", supplier_name != "Unspecified Supplier") %>%
  # Filter out suppliers with extremely low values (optional, for better visualization)
  filter(monetary > 10000)  # Minimum threshold of $10,000

# Preview the RFM data
head(supplier_rfm)
```

### 7.2 Computing RFM Scores and Segmentation

Next, we'll convert the raw metric values into quartile-based scores from 1-4 (4 being the highest value) and create meaningful segments:

```{r}
# Create RFM scores (4 is best, 1 is worst)
...
# Preview the results
head(supplier_rfm_scored)
```

### 3.3 Segment Analysis Summary

Let's create summary statistics for each segment to understand their distribution and characteristics:
...
# Format for display
segment_summary_formatted <- segment_summary %>%
  mutate(
    avg_monetary = scales::dollar(avg_monetary),
    total_monetary = scales::dollar(total_monetary)
  )
...
### 3.4 RFM Heatmap Visualization

We'll create a heatmap showing the distribution of suppliers across the RFM dimensions:

```{r, fig.width=10, fig.height=8}
# Prepare data for heatmap
rfm_heatmap_data <- supplier_rfm_scored %>%
  count(r_score, f_score, m_score) %>%
  # Convert scores to factors for better visualization
  mutate(
    r_score = factor(r_score, levels = 4:1),
...
rfm_heatmap <- ggplot(rfm_heatmap_data, aes(f_score, r_score, fill = n)) +
  ...
  labs(
    ...
    subtitle = "Number of suppliers in each RFM score combination",
...
rfm_heatmap_interactive <- ggplotly(rfm_heatmap) %>%
  layout(title = list(text = paste0("RFM Heatmap: Supplier Distribution<br>",
                                   "<sup>Number of suppliers in each RFM score combination</sup>")))

rfm_heatmap_interactive
```

### 3.5 RFM Bubble Chart

The bubble chart provides a clear visualization of how suppliers are distributed across the RFM dimensions:

```{r, fig.width=10, fig.height=8}
# Create RFM bubble chart
rfm_bubble <- ggplot(supplier_rfm_scored %>% 
                       sample_n(min(n(), 500)), # Sample for better visualization if too many points
                     aes(frequency, recency, size = monetary, color = segment)) +
  geom_point(alpha = 0.7) +
  scale_size(range = c(2, 15), name = "Contract Value") +
  scale_color_brewer(palette = "Set1") +
  # Use log scale for frequency if it has high variance
  scale_x_log10(labels = scales::comma) +
  # Reverse y-axis so lower recency (better) is at the top
  scale_y_reverse(labels = scales::comma) +
  labs(
    title = "Supplier RFM Analysis",
    subtitle = "Bubble size represents total contract value",
    x = "Frequency (Number of Contracts, log scale)",
...
# Convert to interactive plot
rfm_bubble_interactive <- ggplotly(rfm_bubble) %>%
  layout(title = list(text = paste0("Supplier RFM Analysis<br>",
                                   "<sup>Bubble size represents total contract value</sup>")))

rfm_bubble_interactive
```

### 3.6 Segment Distribution Charts

Let's visualize the distribution of suppliers and procurement value across segments:

```{r, fig.width=10, fig.height=8}
# Prepare data for segment distribution
segment_dist <- segment_summary %>%
  select(segment, count, pct_suppliers, total_monetary, pct_spend)

# Create a dual-bar chart showing supplier count vs. procurement value by segment
segment_dist_chart <- ggplot(segment_dist, aes(x = reorder(segment, pct_spend))) +
  geom_bar(aes(y = pct_suppliers, fill = "% of Suppliers"), stat = "identity", alpha = 0.7) +
  geom_bar(aes(y = pct_spend, fill = "% of Spend"), stat = "identity", alpha = 0.7, position = position_nudge(x = 0.3)) +
  scale_fill_manual(values = c("% of Suppliers" = "#1f77b4", "% of Spend" = "#ff7f0e")) +
  labs(
    title = "Supplier Segments: Distribution of Suppliers vs. Procurement Spend",
...
# Convert to interactive plot
segment_dist_interactive <- ggplotly(segment_dist_chart) %>%
  layout(title = list(text = paste0("Supplier Segments: Distribution<br>",
                                  "<sup>Comparing percentage of suppliers to percentage of total procurement value</sup>")))

segment_dist_interactive
```

### 3.7 Supplier Segmentation Data Table

Finally, let's create an interactive data table to explore the supplier segments in detail:

```{r}
# Prepare data for the interactive table
rfm_table_data <- supplier_rfm_scored %>%
  select(
    supplier_name, 
    recency, 
    frequency, 
...
# Create a nicely formatted interactive table
rfm_datatable <- datatable(
  rfm_table_data,
  caption = "Supplier RFM Analysis and Segmentation",
  colnames = c(
    "Supplier Name", "Recency (Days)", "Frequency", "Monetary Value ($)", 
    "R Score", "F Score", "M Score", "RFM Score", "Segment", 
    "Financial Category", "Avg Contract ($)"
  ),
  ...
  options = list(
    ...
    scroller = TRUE,
    columnDefs = list(
      list(targets = c(3, 10), render = JS("function(data, type, row) { return type === 'display' ? '$' + data.toString().replace(/(\\d)(?=(\\d{3})+(?!\\d))/g, '$1,') : data; }"))
    )
  )
) %>%
  formatRound(columns = c("recency", "monetary", "avg_contract"), digits = 0) %>%
  formatStyle(
    'segment',
    backgroundColor = styleEqual(
      unique(supplier_rfm_scored$segment),
      colorRampPalette(brewer.pal(9, "Set1"))(length(unique(supplier_rfm_scored$segment)))
    ),
    fontWeight = 'bold',
    color = 'white'
  ) %>%
  formatStyle(
    columns = c('r_score', 'f_score', 'm_score'),
    backgroundColor = styleInterval(
      cuts = c(1, 2, 3),
      values = c('#f7fbff', '#c6dbef', '#6baed6', '#08519c')
    ),
    color = styleInterval(
      cuts = c(3),
...
### 3.8 RFM Analysis by Ministry or Agency

For a more targeted analysis, we can examine the RFM distribution for suppliers working with specific ministries or agencies:

```{r}
# Function to perform RFM analysis for a specific ministry or agency
rfm_by_entity <- function(data, entity_type, entity_name) {
  # Filter data for the specified entity
  entity_data <- data %>%
    filter(if(entity_type == "ministry") ministry == entity_name else agency == entity_name)
  
  # Calculate RFM metrics for suppliers within this entity
  entity_supplier_rfm <- entity_data %>%
    group_by(supplier_name) %>%
    summarize(
      recency = as.numeric(difftime(analysis_date, max(award_date, na.rm = TRUE), units = "days")),
      frequency = n(),
    ...
    filter(!is.na(supplier_name), supplier_name != "Unknown", monetary > 10000) %>%
    # Create RFM scores
    mutate(
      r_score = ntile(desc(recency), 4),
      f_score = ntile(frequency, 4),
  ...
  # Return the RFM analysis for this entity
  return(entity_supplier_rfm)
}

# Example: RFM analysis for Ministry of Education
moe_rfm <- rfm_by_entity(procurement_enriched, "ministry", "MINISTRY OF EDUCATION")

# Create bubble chart for this ministry
moe_bubble <- ggplot(moe_rfm, aes(frequency, recency, size = monetary, color = segment)) +
  geom_point(alpha = 0.7) +
  scale_size(range = c(2, 15), name = "Contract Value") +
  scale_color_brewer(palette = "Set1") +
  scale_x_log10(labels = scales::comma) +
...
# Convert to interactive plot
moe_bubble_interactive <- ggplotly(moe_bubble)
moe_bubble_interactive
```

### 3.9 Cross-Analysis: Financial Grade vs. RFM Segments

Let's examine the relationship between supplier financial grades and their RFM segmentation:

```{r, fig.width=10, fig.height=8}
# Prepare data for cross-analysis
grade_rfm_data <- supplier_rfm_scored %>%
...
# Convert to interactive plot
grade_segment_interactive <- ggplotly(grade_segment_heatmap)
grade_segment_interactive
```

## 4. RFM Analysis Dashboard UI Design

This UI mockup demonstrates how the RFM analysis module would be integrated into your existing dashboard:

```{r, echo=FALSE}
# This section will be replaced with an actual UI mockup in HTML format
htmltools::HTML('
<div style="border: 1px solid #ddd; padding: 15px; background-color: #f9f9f9; border-radius: 5px;">
  <h3 style="color: #333; margin-top: 0;">RFM Analysis Module UI Design</h3>
  <p>A mockup of the RFM analysis module UI will be provided in the next section.</p>
</div>
')
```
