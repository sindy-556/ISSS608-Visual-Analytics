...
### 8.8 Developing Multi-visualization Interactive Dashboards

Create an advanced dashboard that combines multiple visualization types:

```{r}
# Prepare quarter-based data
quarter_data <- comprehensive_report %>%
  group_by(Product, Quarter) %>%
  summarize(Sales = sum(Sales)) %>%
  group_by(Product) %>%
  summarize(`Quarterly Sales` = list(Sales))

# Join with our existing data
integrated_data <- left_join(
  product_monthly,
  quarter_data,
  by = "Product"
)

# Create advanced multi-visualization dashboard
reactable(
  integrated_data,
  pagination = FALSE,
  ...
  columns = list(
    ...
    `Average Achievement` = colDef(
      ...
      cell = data_bars(
        integrated_data,
        fill_color = colorRampPalette(c("#f8696b", "#ffeb84", "#63be7b"))(10),
        background = "#f0f0f0",
        min_value = 0.7,
        max_value = 1.3
